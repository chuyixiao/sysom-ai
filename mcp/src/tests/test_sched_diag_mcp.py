...
基于agentscope的MCP客户端测试调度诊断工具（delay, loadtask）
"""
import asyncio
import copy
import os
...
class DelayTestParams:
    ...
    threshold: Optional[str] = "20"


@dataclass
class LoadTaskTestParams:
    """loadtask测试参数"""
    uid: str
    region: str
    channel: str  # "ecs"
...
# delay测试用例
DELAY_TEST_CASES: List[DelayTestParams] = [
    # 基础测试 - 默认参数
    DelayTestParams(
...
# loadtask测试用例
LOADTASK_TEST_CASES: List[LoadTaskTestParams] = [
    LoadTaskTestParams(
        uid="1418925853835361",
        region="cn-hangzhou",
        channel="ecs",
    ...
    if toolkit is None:    
        async with _toolkit_lock:
            if toolkit is None:
                ...
                mcp_url = "http://127.0.0.1:7130/api/v1/sched_diag/mcp/"
...
def build_delay_prompt(params: DelayTestParams) -> str:
    """构建delay工具的调用提示"""
    parts = [f"调用delay工具，帮我分析一下{params.instance}实例（region是{params.region}）的调度抖动问题"]
    
    if params.duration and params.duration != "20":
        parts.append(f"，诊断时长为{params.duration}秒")
    if params.threshold and params.threshold != "20":
...
def build_loadtask_prompt(params: LoadTaskTestParams) -> str:
    """构建loadtask工具的调用提示"""
    return f"调用loadtask工具，帮我分析一下{params.instance}实例（region是{params.region}）的系统负载问题，uid是{params.uid}，诊断通道是{params.channel}"
...
async def test_delay(test_cases: List[DelayTestParams]):
    """测试delay工具"""
    logger.info("=" * 80)
    logger.info("开始测试 delay 工具")
    logger.info("=" * 80)
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            prompt = build_delay_prompt(test_case)
...
async def test_loadtask(test_cases: List[LoadTaskTestParams]):
    """测试loadtask工具"""
    logger.info("=" * 80)
    logger.info("开始测试 loadtask 工具")
    logger.info("=" * 80)
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            prompt = build_loadtask_prompt(test_case)
            logger.info(f"调用提示: {prompt}")
...
async def run_all_tests():
    ...
    try:
        ...
        # 测试delay
        await test_delay(DELAY_TEST_CASES)
        
        # 测试loadtask
        await test_loadtask(LOADTASK_TEST_CASES)
