...
基于agentscope的MCP客户端测试网络诊断工具（packetdrop, netjitter）
"""
import asyncio
import copy
import os
...
# packetdrop测试用例
PACKETDROP_TEST_CASES: List[PacketDropTestParams] = [
    PacketDropTestParams(
...
# netjitter测试用例
NETJITTER_TEST_CASES: List[NetJitterTestParams] = [
    # 基础测试 - 默认参数
    NetJitterTestParams(
        uid="1418925853835361",
        region="cn-hangzhou",
    ...
    if toolkit is None:    
        async with _toolkit_lock:
            if toolkit is None:
                ...
                mcp_url = "http://127.0.0.1:7130/api/v1/net_diag/mcp/"
...
def build_packetdrop_prompt(params: PacketDropTestParams) -> str:
    """构建packetdrop工具的调用提示"""
    return f"调用packetdrop工具，帮我分析一下{params.instance}实例（region是{params.region}）的网络丢包问题，uid是{params.uid}，诊断通道是{params.channel}"


def build_netjitter_prompt(params: NetJitterTestParams) -> str:
    """构建netjitter工具的调用提示"""
    parts = [f"调用netjitter工具，帮我分析一下{params.instance}实例（region是{params.region}）的网络抖动问题"]
    
    if params.duration and params.duration != "20":
        parts.append(f"，诊断时长为{params.duration}秒")
    if params.threshold and params.threshold != "10":
        parts.append(f"，抖动阈值为{params.threshold}ms")
...
async def test_packetdrop(test_cases: List[PacketDropTestParams]):
    """测试packetdrop工具"""
    logger.info("=" * 80)
    logger.info("开始测试 packetdrop 工具")
    logger.info("=" * 80)
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            prompt = build_packetdrop_prompt(test_case)
...
async def test_netjitter(test_cases: List[NetJitterTestParams]):
    """测试netjitter工具"""
    logger.info("=" * 80)
    logger.info("开始测试 netjitter 工具")
    logger.info("=" * 80)
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            prompt = build_netjitter_prompt(test_case)
            logger.info(f"调用提示: {prompt}")
...
async def run_all_tests():
    ...
    try:
        ...
        # 测试packetdrop
        await test_packetdrop(PACKETDROP_TEST_CASES)
        
        # 测试netjitter
        await test_netjitter(NETJITTER_TEST_CASES)
